...
module.exports = async function extract(uid, dir, cookie) {
  ...
  let i = 1, page = 1
  do {
    let articlelist = cookie
      ? `http://control.blog.sina.com.cn/blog_rebuild/blog/controllers/articlelist.php?uid=${uid}&p=${i}&status=1`
      : `http://blog.sina.com.cn/s/articlelist_${uid}_0_${i}.html`
    let list = await htmlDownloader(articlelist, { cookie }).catch(console.log)
